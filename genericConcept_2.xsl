...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="SIdoraConcept">
        ...
        <xsl:for-each select="conceptType">
            <dt>Generic Concept Type: <xsl:value-of select="current()"/></dt>
        ...
        <xsl:for-each select="altTitle">
            <xsl:if test="@type !='' or *[normalize-space()]">
        ...
        <xsl:for-each select="timePeriod"> 
            <xsl:if test="@type !='' or intervalname !='' or note !='' or  (string-length(normalize-space(date/@type)) &gt; 0) or (string-length(normalize-space(date/year)) &gt; 0) or(string-length(normalize-space(date/month)) &gt; 0) or (string-length(normalize-space(date/day)) &gt; 0) or (string-length(normalize-space(date/time)) &gt; 0)">
                    <dt>
        ...
        <xsl:for-each select="place">  
            <xsl:if test="@type !='' or geogname !='' or note !='' or  (string-length(normalize-space(point/lat)) &gt; 0) or (string-length(normalize-space(point/long)) &gt; 0) ">
        ...
        <xsl:for-each select="actor">
            <xsl:if test="@role !='' or *[normalize-space()] ">
    ...
    <xsl:template name="formatContactInfo">
        <xsl:if test="*[normalize-space()]">
            <dt>
                <xsl:text>Contact Type:  </xsl:text><xsl:value-of select="@type"/>
            </dt>
        </xsl:if>
    ...
    <xsl:template name="formatPlace">
        <xsl:for-each select="geogname">
               ...
               <xsl:if test="@type!=''">
                   <dd>
                   <xsl:value-of select="@type"/>: <xsl:value-of select="."/></dd>
               </xsl:if>              
        ...
        <xsl:for-each select="boundedby">
                ...
                <xsl:if test="(northlat!='' or westlong!='' or southlat!='' or eastlong!='')">
                    <dd>
                <xsl:text>Bounding Box:</xsl:text></dd>
                </xsl:if>

                <xsl:if test="northlat!=''">
                    <dd><xsl:text>North Latitude: </xsl:text> <xsl:value-of select="northlat"/></dd>
                </xsl:if>

            
                <xsl:if test="westlong!=''">
                    <dd> <xsl:text>West Longitude: </xsl:text> <xsl:value-of select="westlong"/></dd>
                </xsl:if>

                <xsl:if test="southlat!=''">
                    <dd> <xsl:text>South Latitude: </xsl:text> <xsl:value-of select="southlat"/></dd>
                </xsl:if>
                
            
           
                <xsl:if test="eastlong!=''">
                    <dd><xsl:text>East Longitude: </xsl:text> <xsl:value-of select="eastlong"/></dd>
                </xsl:if>
    ...
    <xsl:template name="formatTaxonName">
        ...
        <xsl:if test="scientificNameAuthorship != ''">
         ...
         <xsl:if test="commonName !=''">
              <dd><xsl:text>Common Name: </xsl:text>
